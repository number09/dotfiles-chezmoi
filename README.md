...
Neovim で使う LSP は **「プロジェクトの依存物」ではなく「開発者個人のツール」** と捉え、
どのプロジェクトにいても使え、かつ **どのプロジェクトのランタイムにも依存しない** レイヤに置く。
具体的には **mise のグローバル設定 (`~/.config/mise/config.toml`) に一本化** する。

```
OS (macOS)
├─ Homebrew ……………………… GUI / システム物（LSPは置かない）
│
└─ mise
    ├─ グローバル (~/.config/mise/config.toml) ◀── LSPは全部ここ
    │   ├─ node = "lts" ……………………… npm系LSPの安定ベース
    │   │
    │   ├─ npm:@vtsls/language-server … TypeScript LSP（bin: vtsls）
    │   ├─ npm:basedpyright ……………… Python LSP（bin: basedpyright-langserver）
    │   ├─ aqua:astral-sh/ruff ………… Python（lint/format, ruff server）
    │   ├─ aqua:hashicorp/terraform-ls … Terraform LSP
    │   └─ aqua:LuaLS/lua-language-server … Lua LSP
    │
    │        └─▶ PATH / shims ─▶ neovim (vim.lsp.config)
    │
    └─ プロジェクト (repo/mise.toml) … 各ランタイムのみ（LSPは置かない）
        ├─ node = "20"
        ├─ python = "3.12"
        └─ terraform = "1.9"
```
...
- LSP はすべて `mise グローバル` の1ファイルに集約する。
- `Homebrew` と `プロジェクト mise` には LSP を置かない（役割を混ぜない）。
- `node = "lts"` をグローバル側に置き、npm系LSP（vtsls / basedpyright）の土台にする。
  プロジェクト側で `node = "20"` 等に切り替えても、グローバルの土台は無傷なので LSP は壊れない。
- グローバル mise（L2）とプロジェクト mise（L3）は別ファイル・別レイヤであり競合しない。

**なぜ pnpm global / uv tool / Mason ではないか:**

- pnpm global は紐づく node が曖昧でバージョン切替に弱い。mise の `npm:` バックエンドは
  専用の安定 node に固定されるため堅牢。
- ruff / terraform-ls / lua-language-server は単一バイナリなので aqua でランタイム非依存に入れる。
- Mason（nvim 内蔵）はバージョンが nvim 内に閉じ、再現性・一元管理が効かない。
...
- aqua 系（ruff / terraform-ls / lua）は **cosign / slsa / minisign / github_attestations 検証が
  デフォルト ON**。npm 系（vtsls / basedpyright）が相対的に高リスクなので、upgrade 時は特に慎重に。
