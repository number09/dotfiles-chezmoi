...
BTT（キーボードショートカット / トラックパッドジェスチャ等）の設定本体は
`~/Library/Application Support/BetterTouchTool/` の **ライブSQLite DB**（`btt_data_store.*`）に
保存される。これはバイナリで常時書き換わり、ライセンス等の秘匿情報も含むため
**そのままは管理しない**（`Library` は `.chezmoiignore` 済み）。

代わりに **エクスポートしたプリセット（`.bttpreset` = JSON）を管理する**。

- 管理ファイル: [`~/.config/bettertouchtool/Default.bttpreset`](dot_config/bettertouchtool/Default.bttpreset)
  （BTTが自動で読む場所ではない。純粋なバージョン管理用のエクスポート置き場）
- **設定を変えたら**: BTT設定ウィンドウ左サイドバーのプリセットメニューから
  `Export Preset...` で再エクスポート → `~/.config/bettertouchtool/Default.bttpreset` に上書き → `chezmoi re-add`
- **別マシンで復元**: `chezmoi apply` で `.bttpreset` を配置 → **BTTで手動インポート**（ファイルを開く / Import Preset）

> エクスポートJSONにスクリプトや変数でAPIキー等の秘匿情報を入れた場合は、
> コミット前に `.tmpl` 化 or `.chezmoiignore` を検討する。
